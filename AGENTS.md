# WorkNexus 工程规范（AGENTS.md）

> 本文件是 WorkNexus 的**唯一工程规范真相源**，面向所有 AI 编码代理与人类开发者。
> `CLAUDE.md` 是本文件的同步镜像：**任何修改必须在同一提交中同步两个文件。**
...
v0.1 领域模块：`users`、`projects`、`work_items`、`intake`、`workchat`（含 AgentAction）、`skills`（Skill 注册与调用日志）、`dashboards`、`audit`。
