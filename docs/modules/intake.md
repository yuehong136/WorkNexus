...
| 日期 | PR | 变更摘要 |
| --- | --- | --- |
| 2026-06-15 | PR5（前端 + E2E） | `features/intake`：Key Factory + `useIntakeListQuery`/`useIntakeQuery` + create/accept/reject/markDuplicate/snooze/update mutations（unwrap、meta.suppressToast、按 Key Factory invalidate）+ 错误码 3xxx→i18n 映射 + create/convert zod schema；组件 `IntakeStatusBadge`/`IntakeSourceBadge`（语义 token）、`intakeColumns`（DataTable）、`IntakeFormDialog`（创建）、`TriageSuggestionCard`（只读 advisory 建议 + provenance + "采纳建议"预填）、`ConvertToWorkItemDialog`（accept 表单·suggested 预填·assignee 选择）、`RejectIntakeDialog`/`SnoozeIntakeDialog`/`MarkDuplicateDialog`、`IntakeDetailSheet`（右侧抽屉，三态 + 分诊动作 + startTriage）；`routes/intake-page.tsx` at `paths.intake`（router 懒加载 + 项目详情入口按钮 gated intake.read）；i18n `intake` namespace 四步注册 zh/en + projects.detail.intake + workchat actionType 两条；§5.5 手册新增「AI 建议（advisory）卡片」并同步 AGENTS↔CLAUDE。vitest +2（list query / accept mutation，msw）；Playwright E2E `intake.spec`（提交 → 分诊 → 接受转工作项 → 工作项可见）1 passed；web lint(0 err)/typecheck/test(28)/build 全绿 |
| 2026-06-15 | PR3（MCP + AgentAction） | `intake/mcp.py` FastMCP 子服务器：`list_intake_requests`（read/perm:intake.read，直接执行）+ `create_intake_request`（low_write/perm:intake.create）+ `accept_intake_request`（low_write/perm:intake.triage），后两者经中间件 defer 成 pending AgentAction；`worknexus/mcp.py` `mount(intake_mcp, namespace="intake")`。接入 M5 确认链（直接扩展，无 registry）：workchat `AgentActionType` += CREATE/ACCEPT_INTAKE_REQUEST；`_TOOL_TO_ACTION`/`_ACTION_PERMISSION` += intake 两项（→INTAKE_CREATE/INTAKE_TRIAGE）；`_dispatch` 拆为 `_dispatch_work_item_action` / `_dispatch_intake_action`，后者 import `intake.service`（create→source=ai_chat·source_ref_id=action.id·submitter=requested_by_user_id；accept→reporter_id=requested_by_user_id，result_ref=("work_item", converted_work_item_id)）；身份/作用域取自 action 行非 tool 参数。依赖 `workchat.service→intake.service` 无环。测试 +5（MCP 注册+风险 tag、create/accept 两 low_write defer→pending AgentAction、approve dispatch create→ai_chat intake、approve dispatch accept→intake.service 转工作项+回填）；全套 204 passed，ruff/mypy 全绿 |
| 2026-06-15 | PR2（后端核心） | `IntakeRequest` 模型 + 迁移 `f255da65bbd9`（alembic up/down/check 干净，env.py+conftest.py 注册 intake.models；`UniqueConstraint(converted_work_item_id)` 防重复 accept）；schemas（IntakeSource/IntakeStatus StrEnum + ACTIONABLE/TERMINAL 集合 + Create/Update/Accept/Reject/MarkDuplicate/Snooze In + IntakeOut〔含 suggested_assignee 富化〕）；`triage.py`（`TriageEngine` Protocol + `RuleBasedTriageEngine`〔关键词→type/priority、中英信号、provider/version/reason/confidence provenance、time-free〕 + `get_triage_engine`）；service（create〔同步规则分诊 + 审计〕、list/get〔惰性解 snooze〕、update〔改 title/description 重算建议、status→triaging〕、**原子 accept-and-convert**〔抽 `work_items.service.create_work_item_in_tx` 无 commit core，同一事务建 WorkItem〔source=intake/source_ref_id〕+ 回填 converted_work_item_id + status=converted + 审计〕、reject/mark-duplicate〔校验同项目工作项〕/snooze〔未来时间校验〕；终态守卫 INTAKE_NOT_ACTIONABLE；suggested_assignee 服务端校验项目成员、非成员置 null；AI actor 无 reporter 兜底报错）；deps `require_intake_permission`（项目级解析）；router（list/create + 平铺 get/patch/accept/reject/mark-duplicate/snooze，operation_id 供 orval）；3xxx 错误码 3001/3002/3003；AuditAction intake.create/update/accept/reject/duplicate/snooze；config `intake_triage_provider` + .env.example；api 注册 intake_router。测试 28 项（triage 规则 11、service 状态机/原子/守卫 13、REST 4）；全套 199 passed，ruff/mypy 全绿 |
