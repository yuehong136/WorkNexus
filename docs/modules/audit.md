...
| 日期 | PR | 变更摘要 |
| --- | --- | --- |
| 2026-06-16 | PR3（前端） | `features/audit`：`auditKeys` + `useAuditLogsQuery`（contracts `listAuditLogs` + `unwrap`）；`audit-badges.tsx`（`ActorBadge` 按 type 区分 人〔secondary〕/AI〔brand default〕/系统〔outline〕）；`audit-columns.tsx`（时间·actor Badge·action 原值 mono 按钮开抽屉·resource type:id8·project 名）；`audit-filters.ts`（`AuditFilters` + `emptyAuditFilters`，独立文件避免 react-refresh 误判）；`audit-filter-bar.tsx`（actor_type/action〔全枚举〕/resource_type〔精选〕/project〔来自 /me〕select + 起止日期 + 重置）；`audit-detail-sheet.tsx`（`Sheet`：时间/actor/action/resource/project/requestId/ip + before/after/detail JSON `<pre>` + `detail.skillInvocationId` 链 `/skills` + 「查看完整链路」按钮〔resource=agent_action 时按 resource_type+resource_id 过滤得 propose→approve→execute〕）；`audit-page.tsx`（filter bar + DataTable + 分页 + 三态 patterns）；`paths.audit` + router 懒加载 + app-shell `PermissionGate audit.read` 导航（`nav.audit` 复用既有）；i18n `audit` namespace 四步注册 zh/en。vitest +1（`useAuditLogsQuery` unwrap + actor 名解析，msw）；web lint(0 err)/typecheck/test(32)/build 全绿。 |
