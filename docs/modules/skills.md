...
| 日期 | PR | 变更摘要 |
| --- | --- | --- |
| 2026-06-15 | fix（M5 后续） | M5 已上线，更新 `/skills` 页过期文案：`executable.no` 「需 M5 确认」→「需确认」、skillsSection.description 去掉「M5」（low_write 工具现经 AgentAction 人工确认执行，非待 M5）；zh/en 双语同步 |
...
| 2026-06-14 | PR4（前端 /skills 页） | `features/skills`：Key Factory（catalog/invocations/invocationDetail）；useSkillsQuery/useSkillInvocationsQuery/useSkillInvocationQuery（unwrap，contracts client）；RiskBadge/InvocationStatusBadge（语义 token 变体）；SkillList（反射 Skill→Tool，标 risk + 可执行/需 M5 确认）；skillInvocationColumns（tool/skill/风险/状态/代表用户/需确认/AgentAction[M5 占位]/时间）；SkillInvocationDrawer（右侧 Sheet 详情：摘要/error/代表用户/agent/project/conversation/run）；SkillsPage（三态 + risk/status 原生 select 过滤 + 分页 + 抽屉）；paths.skills + router 懒加载 + AppShell 侧栏入口（PermissionGate skill.read，复用既有 nav.skills 文案）；i18n skills namespace 四步注册（index/i18n.ts ns+AppTFunction/i18next.d.ts）zh/en 双语；query hook 测试 2 项（msw）+ 轻量 E2E（登录→/skills→反射目录可见+空调用态）；web lint(0 err)/typecheck/test(24)/build 全绿，e2e 4 passed |
| 2026-06-14 | PR3（contracts） | `npm run contracts:generate`：listSkills/listSkillInvocations/getSkillInvocation + SkillOut/SkillInvocationOut/SkillToolOut/RiskLevel/SkillInvocationStatus/RepresentedUserOut/ListSkillInvocationsParams（独立 commit） |
| 2026-06-14 | PR2（后端骨架） | `skill_invocations` 模型 + 手写迁移 c3a7f1b9d2e4（FK users/ai_agents/projects/audit_logs，索引 tenant_created/tenant_id/tool_name/represented_user_id；alembic up/down/check 干净，env.py 注册 skills.models）；schemas（RiskLevel/SkillInvocationStatus StrEnum + SkillInvocationOut/SkillToolOut/SkillOut/RepresentedUserOut）；reflection（risk_for_tags/permission_for_tags/skill_code_for_tool 解析 `perm:` tag）；context（MCPCallContext + ContextVar + require_mcp_context）；middleware（纯 helper read/verify server token、read delegation、decide_execution、summarize 脱敏 + SkillInvocationMiddleware.on_call_tool：双 token→风险门禁→begin/finish_invocation，log_db 独立 session，reject/blocked 在 call_next 之前；open_session 可注入测试）；service（begin/finish_invocation 写 skill_invocation + skill.invoke 审计并回填 audit_log_id、list/get_invocations、list_skills 反射）；errors 6001–6005；AuditAction.SKILL_INVOKE；mcp.add_middleware 注册；api 注册 skills_router；回填 work_items/mcp.py（require_mcp_context + 6 工具补 perm: tag，删除 _delegated 自校验）；§5.6 手册新增「MCP tool 鉴权与留痕」条目并同步 AGENTS↔CLAUDE；测试 22 项（helper 矩阵/service/middleware monkeypatch/REST/HTTP 冒烟）；in-memory smoke 改为断言 M4 门禁拒绝；全套 131 passed，ruff/mypy 全绿 |
