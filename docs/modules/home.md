...
# 实现选择：直接复用各域已有 ApiModel Out（camelCase·带 projectId 可深链），不另建精简投影。
HomeSnapshotOut       { myTodos:            HomeCardOut[WorkItemOut],
                        overdue:            HomeCardOut[WorkItemOut],     # 见下：用 WorkItemOut 而非内部 OverdueWorkItem
                        pendingAgentActions: HomeCardOut[AgentActionOut], # 复用 workchat schema
                        recentAiCreated:    HomeCardOut[WorkItemOut],
                        pendingIntake:      HomeCardOut[IntakeOut] }
# WorkItemOut/AgentActionOut/IntakeOut 均为 ApiModel（camelCase）；overdue 项亦为 WorkItemOut
# （overdue 由查询条件保证，无需 daysOverdue 列）。AgentActionOut 含 projectId/actionType/riskLevel/status。
```

> **overdue 用 WorkItemOut 而非 work_items 内部 `OverdueWorkItem`**：`OverdueWorkItem` 是仪表盘内部 schema（**非 ApiModel → snake_case**、且**无 projectId**），不宜直接出现在 home 的 REST 响应（违反 §4.3 camelCase + 无法深链）。故 `list_my_overdue_work_items` 返回 `WorkItemOut`（含 dueAt/projectId），daysOverdue 由前端按需从 dueAt 计算或仅标注「逾期」。
...
| 日期 | PR | 变更摘要 |
| --- | --- | --- |
| 2026-06-16 | PR3（前端 + E2E） | `features/home`：`homeKeys` + `useHomeQuery`（contracts `getHome` + `unwrap`，staleTime 0 使返回工作台即刷新）；`home-page.tsx` 五卡网格（`HomeCard` 通用：标题 + count Badge + 截断行列表/空态；`Row` 深链）——**「待确认 AI 动作」一等公民**置顶/高亮（brand 边框）跨整行；myTodos/overdue/recentAiCreated 行链 `paths.workItems(projectId)`、pendingAgentActions 链 `paths.ai`、pendingIntake 链 `paths.intake`。i18n `home` namespace 四步注册 zh/en。**后端修正**：home overdue 卡原复用 `OverdueWorkItem`（work_items 内部 schema·非 ApiModel→snake_case·无 projectId，不宜直接出 REST），改为 `WorkItemOut`（camelCase·带 projectId 可深链）——`list_my_overdue_work_items` 返回 WorkItemOut、`HomeSnapshotOut.overdue: HomeCardOut[WorkItemOut]`，contracts 重新生成（清理孤立 OverdueWorkItem 产物）。E2E `home.spec`（WorkChat 提案 pending→工作台「待确认 AI 动作」卡可见→批准→`/audit` 见 propose→execute 全链〔action 单元格 button 定位避开 filter option〕→工作台「最近 AI 创建」见该工作项→`/settings/ai` 脱敏展示 client）；修 app-shell 导航改动波及的 auth/projects 既有 spec（成员经 设置→成员；settings sub-nav `我的项目` 避免与主导航 `项目` 同名 + projects.spec `项目` 用 exact）。vitest +1（`useHomeQuery` 五卡 unwrap，msw）；web lint(0 err)/typecheck/test(34)/build + **E2E 8 全绿**；后端 254 全绿、alembic 无漂移。roadmap M8 → 已完成。 |
