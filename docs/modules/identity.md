...
| 2026-06-13 | PR3 | 前端与 E2E 落地：mutator/api-client 改 `credentials: 'include'`（APIError 以 contracts 为唯一定义，web 仅 re-export）+ `unwrap()` 解包约定；`lib/auth/`（authKeys/useMeQuery/useSetupStatusQuery/useHasPermission/PermissionGate，无 zustand auth store）；`lib/query-client.ts` 401 reset me + `meta.suppressToast` 内联错误机制；RequireAuth/GuestOnly 守卫；/login、/setup、/invites/:token 页 + AppShell 用户菜单 + /settings/members（用户表、邀请创建/复制链接/撤销）；patterns 五件套（page-skeleton/empty-state/error-state/confirm-dialog/data-table）+ 11 个语义 token 适配的 ui 原语；i18n 新 namespace auth/settings（zh/en）+ AppTFunction；vitest 13 例（useMeQuery/守卫/登录表单，msw）；Playwright E2E（独立 worknexus_e2e 库 + 8201/5174 端口，覆盖 setup→登出→登录→邀请→激活全链路）；AGENTS/CLAUDE §5.1/§5.4/5.5 手册同步（3 个新条目） |
