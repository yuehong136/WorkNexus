...
| 日期 | PR | 变更摘要 |
| --- | --- | --- |
| 2026-06-15 | PR3（只读 MCP 工具） | `dashboards/mcp.py` FastMCP 子服务器：`get_project_dashboard`（tags `{read, perm:dashboard.read}`，入参 `overdue_limit=10`，项目取自 delegation 绑定而非 tool 参数〔§7.3〕）；`service.get_project_dashboard_snapshot`（编排 summary + workload + 截断 overdue 预览 + 规则洞察，复用领域 read-model，不写 SQL）；`schemas.DashboardSnapshotOut`；`worknexus/mcp.py` `mount(dashboard_mcp, namespace="dashboard")`。read 风险经既有 `SkillInvocationMiddleware` 直接执行 + 写一行 `skill_invocations` 留痕（通用 read 路径已由 system_ping 中间件测试覆盖）。测试 +4（snapshot 组装+overdue 截断；MCP 注册+read tag、无 delegation 拒绝、read 经中间件执行+记 skill_invocation〔risk read/status success/requires_confirmation False〕）；全套 228 passed，ruff/mypy 全绿。无新表/迁移。 |
| 2026-06-15 | PR5（前端 + E2E） | 装 recharts 3.8；`components/patterns/charts/`（`BarChart`/`DonutChart`/`LineChart` 薄封装，纯展示·颜色作 props 传入·tooltip 用 `var(--…)`）+ `lib/chart-colors.ts`（`useChartColors` 订阅 ui-store theme + `getComputedStyle` 读 `--chart-1..8`/`--status-*`/`--brand-primary`）+ `globals.css` 加分类色板 `--chart-1..8`（light/dark + @theme）；`features/dashboard`：`dashboardKeys` Key Factory + `useDashboard{Summary,Workload,Overdue,Insights}Query`（unwrap·staleTime 0 使仪表盘随状态变化刷新）、`DashboardCards`（6 KPI）、`DistributionCharts`（status 环形 + type/priority/source 柱，复用 `workItems:*` 标签）、`TrendChart`（创建/完成双折线）、`WorkloadTable` + `OverdueTable`（分页·columns.tsx）、`InsightCard`/`InsightsPanel`（advisory「仅供参考」+ provenance + 按 kind i18n 模板插值 metrics 渲染双语·detail 留 multirag 槽）；`paths.dashboard` + router 懒加载 + 项目详情「仪表盘」入口按钮（gated `dashboard.read`）；i18n `dashboard` namespace 四步注册 zh/en + `projects:detail.dashboard`；§5.5 手册新增「图表封装」并同步 AGENTS↔CLAUDE。vitest +3（summary/overdue query hooks·chart-colors，msw）；Playwright E2E `dashboard.spec`（创建工作项 → 仪表盘 totalCount=1 + 分布/洞察渲染 → 再建一项 → 数字随之更新到 2）1 passed；web lint(0 err)/typecheck/test(31)/build 全绿，E2E 7 passed |
