...
| M7 | 仪表盘 | `dashboards` | `dashboard` | **已完成**（PR1 设计 A–H；PR2 后端核心：领域 metrics read-model〔work_items/intake〕+ M3 summary 重构复用同口径 + dashboards service/insights/4 REST 端点 + 规则版 InsightsEngine，无新表/迁移；PR4 contracts；PR5 前端：recharts 图表封装〔BarChart/DonutChart/LineChart + lib/chart-colors CSS 变量取色〕+ DashboardCards/分布/趋势/workload/overdue/AI 洞察卡 + 入口按钮 + i18n + E2E 主链路〔创建工作项→仪表盘数字随之更新〕。后端 224 tests、前端 31 tests、E2E 7 全绿） | [dashboard.md](modules/dashboard.md) |
