...
| M7 | 仪表盘 | `dashboards` | `dashboard` | **开发中**（PR1 设计敲定 A–H；PR2 后端核心：领域 metrics read-model〔work_items/intake〕+ M3 summary 重构复用同口径 + dashboards service/insights/4 REST 端点 + 规则版 InsightsEngine，无新表/迁移，224 tests 全绿） | [dashboard.md](modules/dashboard.md) |
