...
| M5 | AI WorkChat + AgentAction | `workchat`（含 agent_actions、AI Adapter） | `workchat` | 开发中（PR1 设计文档：敲定 A–H，提案唯一走 MCP 工具调用、low_write defer 建 AgentAction、approve 实时双重校验 + AI Agent actor 执行、SSE 代理、AIClient 抽象主线 D7、7xxx 错误码；后端/Adapter/前端 PR 待开） | [workchat.md](modules/workchat.md) |
