...
| M6 | Intake | `intake` | `intake` | **开发中**（PR1 设计 A–H + PR2 后端核心〔模型/迁移/规则 TriageEngine/状态机/原子 accept-convert/REST/3xxx/审计〕 + PR3 intake MCP 子服务器 + 接入 M5 AgentAction dispatcher；后端 204 tests 全绿；待 PR4 contracts + PR5 前端/E2E） | [intake.md](modules/intake.md) |
