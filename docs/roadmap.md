...
| M8 | 设置与审计 UI + 工作台完善 | `audit` 读侧 + `settings` + `home` | `settings`、`audit`、`home` | **进行中**（PR1 设计敲定 A–G：审计单端点 tenant 级 / Home 单快照 decision B / Skill 只读执行策略不建表 / Profile 仅改显示名 / 无新表·迁移·MCP·access.py 变更） | [audit.md](modules/audit.md)、[home.md](modules/home.md)、[settings.md](modules/settings.md) |
