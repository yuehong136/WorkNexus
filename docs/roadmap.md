...
| M8 | 设置与审计 UI + 工作台完善 | `audit` 读侧 + `settings` + `home` | `settings`、`audit`、`home` | **已完成**（PR1 设计 A–G；PR2 audit 读侧 GET /audit-logs〔tenant 级·actor/resource/project/action/时间过滤·actor 名解析·AI 链按 resource 过滤〕；PR3 home 模块单快照 GET /home〔decision B 编排·领域补公开 read 函数·登录即可按可访问项目集过滤〕；PR4 settings 端点〔PATCH /me 改显示名+审计·GET /settings/ai-connection 脱敏只读〕；PR5 contracts；PR6 前端审计页〔过滤+DataTable+详情抽屉+AI 链「查看链路」+/skills 交叉链〕；PR7 前端 Settings Lite 多页〔profile/我的项目/AI 连接/Skill 执行策略+sub-nav〕；PR8 前端工作台五卡〔待确认 AI 动作一等公民〕+E2E 闭环〔Home 待确认动作→审计全链→设置脱敏〕。后端 254 tests、前端 34 tests、E2E 8 全绿。无新表/迁移/MCP/access.py 变更） | [audit.md](modules/audit.md)、[home.md](modules/home.md)、[settings.md](modules/settings.md) |
...
| 2026-06-16 | M8 完成：审计查询页 + Settings Lite + 工作台 Home 全链路打通，v0.1 16 步闭环全通（首启→登录→项目→工作项→看板→WorkChat→AgentAction 确认→落库→skill_invocations→audit_logs 全链→仪表盘/工作台更新）。栈式 8 PR；后端 254 / 前端 34 / E2E 8 全绿。**实现修正**：home overdue 卡原拟复用 work_items 内部 `OverdueWorkItem`（非 ApiModel·snake_case·无 projectId），前端集成时改为 `WorkItemOut`（camelCase·带 projectId 可深链），`list_my_overdue_work_items` 返回 WorkItemOut。 | dxl |
| 2026-06-16 | M8 设计敲定 A–G（**audit**：单端点 GET /audit-logs tenant 级、项目过滤用查询参数、沿用 owner/admin 的 audit.read 不下放 project_admin、actor 名按页解析 user/agent/system、AI 链按 resource=agent_action+id 过滤不 denormalize、SkillInvocation 仍留 /skills；**home**：单快照 GET /home + 各卡 {total, top-N}、新建 home 模块沿用 decision B 只编排不 import 他模块 models、领域补公开 read 函数、登录即可按可访问项目集过滤、不做分页端点；**settings**：个人资料仅可改显示名〔PATCH /me + USER_PROFILE_UPDATE 审计〕改密码后置、AI 连接只读脱敏绝不回明文 key、Skill 只读「执行策略」不做真实开关不建 enablement 表、AI 连接读=ai_agent.manage；**全局**：无新错误码〔8xxx 空闲〕、无新表/迁移、无新 MCP 工具、不改 access.py），详见 docs/modules/{audit,home,settings}.md；8 PR 栈式拆分 | dxl |
