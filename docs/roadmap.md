...
| M5 | AI WorkChat + AgentAction | `workchat`（含 agent_actions、AI Adapter） | `workchat` | 开发中（PR1 设计文档敲定 A–H；PR2 后端核心：conversations/messages/agent_actions 模型+迁移+service〔提案/approve 实时双重校验+dispatcher 以 AI Agent actor 执行/reject/list〕+REST〔不含 AI 出站〕+改 skills 中间件 low_write defer 建 AgentAction+7xxx 错误码+AuditAction，143 tests 全绿；PR3 AI Adapter（AIClient 抽象 + parse_sse_frame 纯函数 + FakeAIClient + MultiragAgentCompletionsClient〔待 live-verify〕）+ /workchat/runs SSE 代理 + D6 上下文过滤，166 tests 全绿；PR4 contracts、PR5 前端待开） | [workchat.md](modules/workchat.md) |
| M6 | Intake | `intake` | `intake` | 未开始 | 待建 |
