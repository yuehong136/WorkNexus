...
| M1 | Identity & Access | `identity`（+ `audit` 起步） | `auth`、`settings/members` | **开发中**（PR1 模型/setup/会话、PR2 权限/me/邀请/delegation 已完成，余前端 PR3） | [identity.md](modules/identity.md) |
| M2 | 项目空间 | `projects` | `projects` | 未开始 | 待建 |
| M3 | 工作对象 + Workflow Lite + 看板 | `work_items` | `work-items`、`board` | 未开始 | 待建 |
| M4 | Skills / MCP 骨架 | `skills` | `skills` | 未开始 | 待建 |
| M5 | AI WorkChat + AgentAction | `workchat`（含 agent_actions、AI Adapter） | `workchat` | 未开始 | 待建 |
